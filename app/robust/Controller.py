...
class Controller(Component):
    def __init__(self, sensor,actuator,threshold_u,threshold_l,priority):
        ...
        self.thresh_u = threshold_u # 2.59e06   # upper threshold for load
        self.thresh_l = threshold_l # 2.4e06 # lower threshold for load
        self.retrycountu = 0
        self.retrycountl = 0
...
#         self.updatestatus.send_pyobj({self.priority : self.status})
        if self.pending == 0:
            if self.status == '':
                msg = ['qry', (self.aObj,'phase_A_state',self.aUnit)] # query the last known switch status
            else: 
